...
SERVER_DIR="$WORKSPACE_DIR/pyworker"
...
then
    ...
    which pip > /dev/null 2>&1 || apt-get install -y --no-install-recommends python3-pip
    which uv > /dev/null 2>&1 || PIP_BREAK_SYSTEM_PACKAGES=1 pip install --no-cache-dir uv
